...
import { PRIORITY_COLORS, PRIORITY_LABELS, STATUS_COLORS, STATUS_LABELS, TAG_COLORS, formatDate, isOverdue } from '../lib/task-utils';
...
export default function TaskCard({ task, onEdit, compact = false }: Props) {
  ...
  async function handleStatusCycle() {
    const order = ['todo', 'in_progress', 'done', 'cancelled'] as const;
    const idx = order.indexOf(task.status as any);
    const next = order[(idx + 1) % order.length];
    await updateTask(task.id, { status: next, completedAt: next === 'done' ? Date.now() : null });
  }
    ...
    <div className={`ios-card overflow-hidden ${isDone ? 'opacity-60' : ''}`} onClick={() => onEdit(task)}>
        ...
        <div className="flex-1 p-3.5">
          ...
          <div className="flex items-center gap-3 mt-2 ml-8 text-[11px] text-slate-500 dark:text-slate-400">
            <button
              ...
              className={`px-2 py-0.5 rounded-full ${statusColor.bg} ${statusColor.text} font-medium`}
            >
      ...
      {showActions && (
        <div className="flex border-t border-slate-100 dark:border-slate-800 fade-in">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); setShowActions(false); handleStatusCycle(); }}
